...
```{r}
lda_fit <- read_rds('modelos/MIA_lda.rds')
```
...
## Temporal, df viejo
df <- read_rds('../data/txt/MIA.RDS') 

fechas <- read_csv('../data/txt/fechas.csv')
...
fechas <- fechas %>% distinct(autor,titulo, .keep_all = TRUE) %>% 
  mutate(titulo = limpiar_textos(titulo))

df <- df %>% 
  filter(tipo=='notas')

sum(duplicated(fechas[,c("autor", "titulo")]))

df <- df %>% 
    left_join(fechas,by = c("autor", "titulo"))
## Temporal, df viejo
```
...
```{r}
topicos_tsne <- tsne(dist_topicos[3:12],k = 2)
...
```{r  fig.height=10, fig.width=10}
topicos_pca <- princomp(dist_topicos[3:12], cor = TRUE)
...
dist_topicos %>% 
  ...
  gather(topico, valor,2:11) %>% 
  mutate(topico=factor(topico, levels = 1:10)) %>% 
...
adjMat = cosine(t(as.matrix(dist_topicos_autor[,3:11]))) 
...
```{r}
#la paso a dicotomica, no quiero que me quede muy densa, asi que pongo como punto de corte alguito apenas el tercer cuartil
adjMat[adjMat>0.89] <- 1
adjMat[adjMat<0.89] <- 0
